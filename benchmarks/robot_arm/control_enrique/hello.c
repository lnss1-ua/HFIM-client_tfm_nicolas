...
int main() {
    ...
    do {
        ...
        if (target_alcanzado && movimiento_terminado) {
            uart_putc('S');
            fim_exit(0);
        } 
        else if (target_alcanzado) {
            uart_putc('F');
            target_position[0] = 1.2;
            target_position[1] = 4.9;
            target_position[2] = 0.6;
            target_position[3] = 0.0;
            movimiento_terminado = 1;
            target_alcanzado = 0;
        } 
        else {
            uart_putc('K');
    ...
    } while (!target_alcanzado || !movimiento_terminado);
